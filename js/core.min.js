 ...
 *
 */
(function(){(function($,document,window){var Calculator;return Calculator=(function(){function Calculator(element){this.$element=$(element);this.initialization()}Calculator.prototype.initialization=function(){var ctx;ctx=this;ctx.$element.on("click keyup",function(event){var PriceDist=parseInt($(".distance").attr("data-price"));var PriceWeight=parseInt($(".weight").attr("data-price"));var PriceCapacity=parseInt($(".capacity").attr("data-price"));var distValue=$("#dist").val();var weightValue=$("#weight").val();var PriceLength=$("#length").val();var PriceHeight=$("#height").val();var PriceWidth=$("#width").val();var paramFragileYes=parseInt($("input#blankRadioYes:checked").attr("data-price"));var paramExtra1=parseInt($("input#extra1:checked").attr("data-price"));var paramExtra2=parseInt($("input#extra2:checked").attr("data-price"));var paramExtra3=parseInt($("input#extra3:checked").attr("data-price"));if($.isNumeric(distValue)){if(!$.isNumeric(paramFragileYes)){paramFragileYes=0}if(!$.isNumeric(paramExtra1)){paramExtra1=0}if(!$.isNumeric(paramExtra2)){paramExtra2=0}if(!$.isNumeric(paramExtra3)){paramExtra3=0}var totalDist=(PriceDist*distValue);var totalWeight=(PriceWeight*weightValue);var totalCapacity=(PriceCapacity*((PriceLength*PriceHeight*PriceWidth)*0.0001));var total=totalDist+totalWeight+totalCapacity+paramFragileYes+paramExtra1+paramExtra2+paramExtra3;var newTotal=Math.round(total);$("#total").html(newTotal)}});return ctx};$.fn.extend({Calculator:function(options){return this.each(function(){var $this;$this=$(this);if(!$this.data("Calculator")){return $this.data("Calculator",new Calculator(this,options))}})}});window.Calculator=Calculator;return Calculator})()})(window.jQuery,document,window)}).call(this);